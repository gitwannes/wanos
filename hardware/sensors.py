...
async def mock_temperature_sensor(state_manager: StateManager) -> None:
    """
    Phase 2 Lab Mode: Simulates a physical temperature probe.
    Gradually warms up if the sauna is active and emitting PWM.
    """
    await state_manager.logger.info("🔬 [LAB MODE] Mock temperature sensor started.")
    current_mock_temp: float = 20.0  # Start at room temperature
    ...
    while True:
        try:
            ...
            if state.sauna.active and state.sauna.modulation_pwm > 0:
                ...
                heating_factor: float = (state.sauna.modulation_pwm / 100.0) * 0.5
            ...
            await state_manager.logger.error(f"Mock sensor error: {e}")
