...
| **C25 — Overlay dew likelihood + compare** | ✅ OWM dew%; compare; Admin **Outside weather** — **Pi smoke 2026-09-01** | History + Admin · mid |
...
Explorer History 60s auto-refresh (`refreshExplorerHistory` → soft `reloadSelectedSensorDetail`) keeps series in the chart box (no title + dark-empty plot). Settings + titled windows (last 24h / month / year) and zoom/selection kept. Hard open/switch unchanged. **C16** keeps this no-wipe refresh.
...
## 📋 C16 — Day chart sliding 24 h window ✅ DONE

**Origin:** operator inbox **2026-08-12**. Explorer History day panels. Size **mid**. **Combined ship with C24**.
...
**Shipped summary:** Day `range=day` returns **`hires_days`** buffer; FE default **24 h** viewport (zoom-out to full retention, zoom-in ~1 h); pan; soft-refresh pan/live pin; from/to subtitle when not live; water day = hourly bars over `hires_days` + same pan UX.

**Product reference:** [`docs/sensor_history.md`](../sensor_history.md) §§ day charts / retention / gap-break.

**Out of scope:** windowed API per pan; changing retention; month/year; `sensorhistory.html`; overlay extras → **C24**.

**C16 DoD:** ✅ **2026-08-23** — code + docs + **Pi smoke** (operator). Closed with **C24**. **Last DoD docs audit** this close-out.

---

## 📋 C24 — Temp/hum day fullscreen + extra climate lines ✅ DONE

**Origin:** operator inbox **2026-08-16**. Explorer History temp/hum. Size **mid**. **Combined ship with C16**. Kickoff locks **2026-08-16** / CI formula **2026-08-17**; residual locks **2026-08-23**.

**Operator reconfirm (verbatim, 2026-08-23):**
...
**Shipped summary:** Button **Open detail in full screen** → tab overlay (not F11); AH + **Feels-like humidity**; 5 checkboxes; 3rd y-axis g/m³; CSV full `hires_days`; inherit inline pan; overlay frost; mobile chrome.

**Product reference (formulas + axes):** [`docs/sensor_history.md`](../sensor_history.md) § Day overlay (C24).

**Out of scope:** inline extras; month/year overlay; F11; xlsx; dew likelihood / compare → **C25**.

**C24 DoD:** ✅ **2026-08-23** — code + docs + **Pi smoke** (operator). Closed with **C16**. **Last DoD docs audit** this close-out.

---

## 📋 C25 — Overlay dew likelihood % + compare ✅ DONE

**Origin:** operator inbox **2026-08-16** (+ compare **2026-08-23**, Admin weather **2026-08-27**). Size **mid**. Sequence: **after C24**. Kickoff **2026-08-23**; implement + Admin panel locks **2026-08-27**. **Shipped 2026-08-27**; **Pi smoke 2026-09-01**.
...
**Shipped summary:** OWM every climate poll → `dew%` samples + RAM snapshot; day API `series.dew_likelihood`; overlay 6th checkbox **Dew likelihood %** + **Compare with** peer climate + CSV; Admin **Outside weather** panel (sun moved from General Diagnostics).

**Product reference:**
* Formula + rain/night rules: [`env-schedule-and-system-events.md`](../env-schedule-and-system-events.md) §9
* Charts + compare UX: [`sensor_history.md`](../sensor_history.md) § Day overlay extras (C25)
* OWM loop: [`reference.md`](../reference.md) (`open_weather.py`)

**C25 DoD:** ✅ **2026-09-01** — overlay dew%; compare; Admin Outside weather; `/api/state` last-poll snapshot; **Pi smoke** (operator). **Last DoD docs audit** this close-out.
...
* **C24:** ✅ **Pi smoke 2026-08-23** — tab overlay; AH / Feels-like; CSV; inherit pan; frost. Product: [`sensor_history.md`](../sensor_history.md) § Day overlay.
* **C25:** ✅ **Pi smoke 2026-09-01** — dew% storage; overlay compare + Admin **Outside weather**. Product: [`sensor_history.md`](../sensor_history.md) § Day overlay extras; [`env-schedule-and-system-events.md`](../env-schedule-and-system-events.md) §9.
...
**Product reference (canonical):** [`sauna-ir.md`](../sauna-ir.md) §3.7 (LCD log + compose), §4 (analytics, Admin/WISC session UX, GPIO arm gate).

**Shipped (summary):**

| Area | Outcome |
|------|---------|
| **Analytics** | Time-weighted segmented IR learn (mod plateaus); calc Wh gap fix (`_last_pulse_ts` reset + calc `delta_t` cap); `learn_count_*` from session audit rows |
| **Logging** | `KWH_PULSE` silent in `wanos.log`; LCD agent semantic dedupe (no countdown-only DEBUG); L3 log `§1` → `°C` |
| **LCD compose** | IR line1 @ 100% uses `right_align_timer` |
| **Admin** | VT323 LCD mirror; Calc W (V-adj) label; IR-only live energy Wh; GPIO arm status (`OFFLINE` … `READY` → `ARMED`); last session detail line; dropped `updated:` row |
| **WISC** | Hide setpoint/IR mod sliders when idle; last session = smart when only |
| **Sensors** | Wake poll + 2 s fast cadence until `sauna_calc_temp` after SHT11 arm |
| **Session when** | Dutch: `vandaag`/`gisteren`/`eergisteren` + `ochtend`/`middag`/`namiddag`/`avond`; older dates `nl-BE` month abbrev + `HH:MM` |

**Out of scope (unchanged):** **C33** History charts; **L2** LCD on WanOS Pi.

**C34 DoD:** [x] Shipped per locks · [x] Product docs updated (`sauna-ir.md`, `reference.md`) · [x] Last DoD docs audit **2026-09-02**.
...
> add IR & Sauna to history: detail with % MOD, month & year with #hrs and #days, same as eg Sonos
>
...
**Product reference (canonical):** [`sauna-ir.md`](../sauna-ir.md) §4–5 (Real vs Calc, element DB, Admin/WISC/History UX); [`sensor_history.md`](../sensor_history.md) (meter Wh baseline, leak NVRAM); [`reference.md`](../reference.md) (`element_power_store.py`, `power_analytics.py`).

**Shipped (summary):**

| Area | Outcome |
|------|---------|
| **C31** | IR calc energy; backend sauna/IR mutual exclusion; IR mod history fix; R_th N/A + retain; session history IR columns + avg W; admin session delete; WISC/kiosk banner cleanup (later refined in C32) |
| **C32** | IR hardware PWM (`lgpio.tx_pwm`); `element_power_w` DB + EMA learning + session audit; removed `effective_watts_*` config + Admin edit API; Admin Power & Thermal redesign; LCD line1 C32 format; IR default 7 min + `IR_TIMER_ADJUSTED`; Session History Wh/kWh + fixed ℹ popover; cumulative Total kWh (NVRAM 11001 + baseline); leak W NVRAM persist; sauna/IR learn counts + last-learn Admin; WISC live session + last-session one-liners |

**Out of scope (unchanged):** richer sauna session table columns; historical session row repair; B17/B18.

**Verbatim / kickoff detail:** archived in git history for this file pre-**2026-08-30** close-out.
...
*(none — closed **2026-08-23** incl. Pi smoke.)*
