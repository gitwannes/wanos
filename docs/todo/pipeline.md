...
13  hold   low  C11          Control vs History list membership (assess)        phaseC § C11
14  open   mid  C12          Post-C10 polish (+ Hidden preset admin-only)       phaseC § C12
15  hold   low  C17          Alert banner dismiss vs reload (assess)            phaseC § C17
16  open   mid  C16          Day chart sliding 24 h over hires_days             phaseC § C16
17  open   mid  C24          Temp/hum day fullscreen + AH/CI + CSV              phaseC § C24
18  open   mid  C25          Overlay dew-likelihood % (after C24)               phaseC § C25
...
22  open   mid  G6           bugfix: Scoped CONFIG_RELOAD + Save config         phaseG § G6
...
```
Near-term: **B7 → B8 → B22**. **E** may run beside B7–B8. Vendor bridges **G11→G9→G10→G12→G13** after **G4**, before **F**.
