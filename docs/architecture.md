...
* **Handling Disconnects (WiFi Drops & Broker Deaths):**
    * The frontend listens for WebSocket/SSE connection drops.
    ...
    * Upon reconnect, the frontend fires a REST/WebSocket intent to request a full state dump from the backend to instantly synchronize, bypassing reliance on potentially missed stream updates.
...
* **State Subscription Strategy:** A central **Pinia store** manages all SSE/MQTT subscriptions.
Vue components read reactive state strictly from the store and never subscribe to streams directly.
...
If the process exits or crashes, a guaranteed teardown sequence pulls SSR pins LOW, zeroes PWM channels, kills the SSE stream, and releases the heartbeat pin to immediately shut off heating elements.
...
*(For full payload schemas, refer to `reference.md`)*

**Hardware Events:** `TEMP_UPDATED`, `HUMIDITY_UPDATED`, `WATER_PULSE`, `KWH_PULSE`, `DOOR_CHANGED`, `SENSOR_ERROR`, `TIMER_TICK`

**Sauna Events:**
`SAUNA_ON`, `SAUNA_OFF`, `SETPOINT_CHANGED`, `MODULATION_UPDATED`, `SETPOINT_REACHED`, `SAUNA_HOLD`, `SAUNA_TIMER_EXPIRED`, `HOLD_TOGGLED`, `TIMER_ADJUSTED`, `VENT_WAIT_EXPIRED`, `VENT_RUN_EXPIRED`
...
* **Phase 3: Environmental State Machine (COMPLETE)**
  Door interlocks, Session Timers, Auxiliary Controller (Hue/LCD simulation), and Thermodynamics Lab Engine. SSE implementations for real-time frontend bridging.
* **Phase 4: Vue.js Frontend & External Integrations (ACTIVE)**
