...
* **Warm Storage (Local Filesystem):** Critical system states and structural configuration models read directly from highly readable YAML profiles (`config.yaml`, `config_hardware.yaml`, `config_lab.yaml`, `config_zwave.auto.yaml`, `automations.auto.yaml` — rules, soft-hide, auto-off, **`device_product_types`**, and the UUID `events:` catalog). **B14 part 1 ✅:** automation rules may carry **`cooldown`**, Compare **`for`** (sustained-for), and Set **`timing`/`duration`/`end`** (timed Set; Hue end may use `color_mode: revert`); bathroom vent automation OFF on `zwave.vent.badk_1e` respects hub lock **90001** (re-eval on lock expiry). Stable logical device ids live in system-owned `entity_registry.auto.yaml` (auto-maintained; not mixed into commented `config.yaml`). Z-Wave / RFX binary actuators use **`zwave.*`** / **`rfx.*`** (vent motors **`zwave.vent.*`**). Explorer dashboard buttons come from `dashboard_events` (built from user `events:` with show ∧ enabled listener) — not from `scene.*` entity rows.
...
* **Cold Storage (System Logs):** Structured diagnostic traces via Loguru multi-sink (`wanos.log`, `wanos_debug.log`, `wanos_automations.log`, `wanos_iwhw.log`, …). Operator tail: repo-root `wanoslog.sh` — e.g. `./wanoslog.sh log 4` (automation, DEBUG omitted) or `./wanoslog.sh log 4 debug` (all levels incl. X-RAY).
