...
*   **Stations dictionary:** `config.sonos.stations` (TuneIn key → URI) is also published on `/api/state` as `system.sonos_stations` for Blocky station pickers (Phase B6C), mirroring `system.hue_presets`.
