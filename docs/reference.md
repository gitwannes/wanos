...
* `app.js`: Master Alpine.js reactive interface store managing SSE channel bindings, connection watchdogs, dynamic client-side uptimes, local UI layout persistence, and JWT role routing. **C27:** `sunriseDisplayText` / `sunsetDisplayText` (local HH:MM + relative) for Admin General Diagnostics and Explorer ℹ popover. **Admin:** `chronologicalTimeline` + `formatPlannedTimelineLabel()` — **`Scene: {name}`** rows (G15 ship).
