...
* Explorer History (**C10** / **C12 ✅** / **C19**): omits `type === "scene"` catalog-event rows from the History list; actuator charts use binary ON/OFF (day) + **duration ON** month/year; **Hue + audio** month/year duration ON (no events/min/max); blinds Level min/max + Events; motion hits; duration Y-axis ~5 ticks (month±10 / year±1 h); day climate frost when temp &lt; dew; dew off month/year; Planned Automations drops past/done timers (no stale `imminent`). Open detail auto-refreshes every **60s** (tab visible): soft `setOption` `{ notMerge: false, replaceMerge: ['series', 'dataZoom'] }` (+ `yAxis` for duration charts); rebind ECharts if Alpine replaced the DOM node; hard open/switch unchanged.
