...
* **`GET/POST/PUT/DELETE /api/automations`** | Admin CRUD. **Persists B19 branches** (`branches: [{ when, conditions, actions }]`, `name`…`id` last). Legacy `trigger`+`cases` **hard-rejected** on write. **B4/H4 ✅:** `conditions` may nest Logic groups `{ op: and\|or\|not, children: [...] }` alongside leaf Compares; top-level branch list = implicit AND. GET returns branch rows (**SR `name` normalized to companion SE catalog**). **B10H:** GET does **one** YAML round-trip (bind uses in-memory events map — no N+1). Each write dispatches `CONFIG_RELOAD_REQUESTED`. Event Compare / fire-actions store **event UUID** (see `docs/todo/phaseB-blocky.md` § B10B / § B19 / § Ship B4). Pre-B10B schedule families (`SCHEDULE_WINDOW_EDGES`) removed on that cutover. **B10F:** successful CRUD also writes `INFO` lines to `wanos.log` with **quoted** names (`user rule "…" changed`, etc.). System-rule titles always bind to the SE catalog on POST/PUT; boot merge rewrites drifted YAML free-text. **B9C / B19:** Blockly authors `device_state` conditions with `attribute: position` (shutters; UI open %, YAML closed %, inequalities flipped) or `attribute: volume` (Sonos/Onkyo); dual `temp_hum` uses `attribute: temperature` \| `humidity`; motion Compare is wake-only (`is: ON`). **G5 ✅:** operator UE/UR **`Cinema rolluik half`** on dashboard (`show_on_dashboard`) — if cinema open % **> 50** → set **50%** open (detail → [`phaseG-integrations.md`](todo/phaseG-integrations.md) § G5).
