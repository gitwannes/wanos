...
* `wanos-nvram.json`: Atomic Non-Volatile Memory (NVM) store bypassing `log2ram` to persist cumulative hardware metrics (e.g., liters, absolute house Wh on `11001`) and the idle leak baseline (`p_leak_baseline_watts`) across unexpected power losses.
