...
* `wanos-nvram.json`: Atomic Non-Volatile Memory (NVM) store bypassing `log2ram` to persist cumulative hardware metrics (e.g., liters, absolute sauna-circuit Wh on `11001`) and the idle leak baseline (`p_leak_baseline_watts`) across unexpected power losses.
...
* `helpers/smtp_test.py`: Standalone Gmail SMTP probe (`ssl://smtp.gmail.com:465`, AUTH LOGIN). Edit `USER` / `APP_PASSWORD` / `TO` in-file; aborts with WARNING if the password looks like a placeholder (`xxxx xxxx xxxx xxxx`, deny-list, all-same-letter). Plain ASCII console; never prints the password or its base64. **Keep functional parity** with sibling scripts in the **hofmans.be** repo: `smtp_test.php` (primary twin) and `smtp_net_test.php` (net/auth variant — **no** Python twin in WanOS). When changing probe behaviour or the placeholder gate, update both repos.
...
* `product_type_policy.py`: Resolved product type (`light`|`switch`) — Hue forced light; overrides from `device_product_types`; birth default switch. Stamped onto live `device_metadata` at core rebuild, again after Z-Wave lazy map, and on metadata rewrite (`HUB_STATE_CHANGED`) so Explorer filters stay aligned with Timers & types.
...
* `state_manager.py`: The ultra-fast core engine driving the unidirectional event execution loop. It acts as a pure memory router, delegating payload processing to the Strategy Pattern registry. **B10J:** INFO `Event Received […]` uses catalog display name for system/user catalog events (unknown UUID stays raw; internals unchanged).
* `sse_hub.py`: Event-driven SSE fan-out — domain deltas on queue drain + immediate connect **ping** + **5 s** quiet ping (**B10H** / **C23**). On subscribe, **seeds** current domain snapshots (**C38**) so Admin GPIO/SHT11 LIVE/READY is not stuck if one-shot bus-health was missed. `SseClient` is `@dataclass(eq=False)` (hashable in the client `set`). Also pushes `c18_commit` per-idx apply/revert frames (**C18**).
...
* `hub_handlers.py`: Handles generic device state mutations and advanced Hue color payload dictionaries. **Doors:** `handle_door_changed` updates `devices[idx]` and reconciles `state.doors.*_since_unix` on every `DOOR_CHANGED` (seed missing same-mode stamp; clear opposite).
...
* `app.js`: Master Alpine.js reactive interface store managing SSE channel bindings, connection watchdogs, dynamic client-side uptimes, local UI layout persistence, and JWT role routing. **S1:** `saunaDoorClosedTooLong()` for WISC/kiosk start gate (`doors.sauna_closed_since_unix` vs `sauna.door_closed_max_mins`). **Admin Site info:** door rows via `formatDoorAdminRow` / ticker; SSE domain **`doors`**. **Admin Sauna/IR:** `formatRthInsulation()` renders **Rth** as `0.00 °C/kW` (stored °C/W ×1000). Session runtime display **HH:MM:SS**. **C27:** `sunriseDisplayText` / `sunsetDisplayText` (local HH:MM + relative) for Admin General Diagnostics and Explorer ℹ popover. **C38 / C25:** `owmLastPollDisplayText` + `gpioOutputArmStatusText` updated on the same 1 Hz ticker (not Alpine getters that skip `Date.now()`). **Admin:** `chronologicalTimeline` + `formatPlannedTimelineLabel()` — **`Scene: {name}`** rows (G15 ship). **C37 ✅:** Page Lifecycle resume (`visibilitychange` / `resume` / bfcache `pageshow`) force-closes zombie EventSource and reconnects; snapshot fetch abort timeout 20 s; offline overlay must not use `x-cloak` (OnePlus 12 smoke **2026-09-09**). **B10L:** `offlineStatusLine` milestones under NOT CONNECTED. **C20:** Clear All = `dismissBellAlert` on every visible bell row. **C21:** Explorer `auto_off_countdown` only when device `isOn`.
* `wanos-shell.js`: Shared offline overlay, app nav, light modal, Admin bell. **B10L:** default offline copy **Re-connecting to WanOS...** + milestone; Timers/Hidden keep `Loading…`. **C20:** Clear All when `bellAlerts.length > 0`. **B10M:** Save Hue preset not blocked by matching active chip colour.
...
* Explorer History (**C10** / **C12 ✅** / **C19**): omits `type === "scene"` catalog-event rows from the History list; actuator charts use binary ON/OFF (day) + **duration ON** month/year; **Hue + audio** month/year duration ON (no events/min/max); blinds Level min/max + Events; motion hits; duration / utility / host Y-axis **at most 8** tick labels (month±10 / year±1 h for duration); day climate frost when temp &lt; dew; dew off month/year; Planned Automations drops past/done timers (no stale `imminent`). Open detail auto-refreshes every **60s** (tab visible): soft `setOption` `{ notMerge: false, replaceMerge: ['series', 'dataZoom'] }` (+ `yAxis` for duration charts); rebind ECharts if Alpine replaced the DOM node; hard open/switch unchanged.
* Alert UI dismiss log (**C8** / **C12**): `Alert dismissed (banner|bell): level=… produced_at=YYYY-MM-DD HH:MM:SS "…"` (same stamp shape as loguru line time).
* Z-Wave config search (**C12**): multi-term AND + `-term` exclude on name / path / idx (same as Explorer).
* Explorer blinds ui-lock (**C12**): proportional to travel Δ% × `blinds.travel_times` / `default_travel_time_secs` × 1.10 (mirrors hub debounce); travel config on `system.blinds_*`.
* Manage Presets **Hidden** row (**C12**): admin-only (chip + modal).
...
* `actuators.py`: GPIO outputs — safety contactor arming; sauna U/V/W **software PWM** at `config.sauna.pwm_freq` (default **0.5 Hz** so 1% MOD = one full 50 Hz sinus for ZC SSRs); IR **`lgpio.tx_pwm`** with stepped duty/freq (see [sauna-ir.md](sauna-ir.md) §1.1a).
...
* `lcd_screen1.py`: Shared 16×2 WISC-compatible composer for sauna LCD screen1 (`wanos/lcd/screen1` + `sauna.lcd_line1/2`). **L4 / C38:** pre-arm remaining = duration seconds (not unix); mm:ss only when remaining &lt; 15 min; door-closed duration right-aligned on line2 when closed; door **open** = `plz close sdoor` with **no** duration.
...
* `power_analytics.py`: Sauna/IR session energy accounting (Real vs Calc), NVRAM leak restore (frozen while sauna vent ON), `element_power_w` EMA learning (sauna full-load = contiguous U/V/W ≥95% ≥30 s + Wh while in window; IR segmented plateau C34), session SQLite persistence; owns sauna session sample telemetry flush. Boot: `_init_sqlite()` before `load_row()` so a missing `sauna_sessions.db` does not crash on first-boot schema ALTER. **C38+:** MOD=0 Real W display gate + unexpected meter warning when leak-subtracted load &gt; 1 W (raw &gt; leak + 1 W).
...
* **Payload Examples:**
  * Baseline Activation: `{"active": true, "setpoint_temp": 80.0, "modulation_pwm": 0, "phases_pwm": {"U": 0, "V": 0, "W": 0}, "fireorder": "UVW"}`
  * Delta Power Shift: `{"modulation_pwm": 75, "phases_pwm": {"U": 25, "V": 100, "W": 100}}`
...
* **`GET /api/debug/entity-registry-check`** | Admin-only. Runs `run_entity_cutover_checks` (plus live `device_metadata`) and returns JSON including annotated `report_text` for the Admin Debug modal. Blocky also calls this after automation Save/Delete and shows GREEN/RED in-page. **B19:** flags leftover `trigger`+`cases` rows (`re-author on If/Do branches`). **B4/H4 ✅:** zero legacy OR-list expected — GREEN after cutover.
...
* **`GET/POST/PUT/DELETE /api/automations`** | Admin CRUD. **Persists B19 branches** (`branches: [{ when: if\|else_if, conditions, actions \| then }]`, `name`…`id` last; **bare `when: else` rejected** — Else-if only). **B22 ✅:** branch **`actions` xor `then:`** — top **If/Then** wraps nested **If/Do/Else-if/Do** (`then: { branches, leading_actions?, trailing_actions? }`); optional **leading/trailing Sets** before/after inner chain; inner device Compares = **level/time only** (no `is: ANY`, no wake device, no event Compare); **`MAX_NEST_DEPTH = 3`** for `then` + Logic groups. Legacy `trigger`+`cases` **hard-rejected** on write. **B4/H4 ✅:** `conditions` may nest Logic groups `{ op: and\|or\|not, children: [...] }` alongside leaf Compares; top-level branch list = implicit AND. **B14 part 1 ✅ (Pi smoke 2026-09-09):** optional rule **`cooldown`** (`HH:MM:SS`, 1 min…4 h; arm INFO, skip-while-cooling DEBUG/X-RAY); Compare leaf **`for`** (sustained-for / level hold, not edge-cross; incompatible with `is: ANY`); Set action **`timing: for|after`** + **`duration`** + **`end:`** (for = start now + explicit end Set, default OFF; switches/audio end fixed OFF; Hue end OFF | `color_mode: revert` | set color; after = H2 delayed start); same-device timed Set **replaces**; timers **RAM-only** (drop on restart). Engine defers automation **OFF** on `zwave.vent.badk_1e` while hub lock idx **90001** is active (re-eval on `BATH1_VENT_LOCK_EXPIRED`). Blockly: Set **immediately/for/after**, Compare **sustained duration**, editor **Rule cooldown** (`<not configured>` overlay when empty). Cache `blocky.js?v=56`. Blockly toolbox: **Control · Logic (and/or) · Conditions · Actions** (no Time drawer / no NOT / no bare Else). Device defaults: Hidden OFF → `zwave.buro_licht` ON; Hidden ON → `sensor.door.sauna_deur` opens / `zwave.vent.sauna` ON. System event pickers omit telemetry UUIDs in `NON_PICKABLE_SYSTEM_UUIDS` (Hub/Door/Temp/… — use device Compares or schedule SEs). GET returns branch rows (**SR `name` normalized to companion SE catalog**). **B10H:** GET does **one** YAML round-trip (bind uses in-memory events map — no N+1). Each write dispatches `CONFIG_RELOAD_REQUESTED`. Event Compare / fire-actions store **event UUID** (see `docs/todo/phaseB-blocky.md` § B10B / § B19 / § Ship B4). Pre-B10B schedule families (`SCHEDULE_WINDOW_EDGES`) removed on that cutover. **B10F:** successful CRUD also writes `INFO` lines to `wanos.log` with **quoted** names (`user rule "…" changed`, etc.). System-rule titles always bind to the SE catalog on POST/PUT; boot merge rewrites drifted YAML free-text. **B9C / B19:** Blockly authors `device_state` conditions with `attribute: position` (shutters; UI open %, YAML closed %, inequalities flipped) or `attribute: volume` (Sonos/Onkyo); dual `temp_hum` uses `attribute: temperature` \| `humidity`; motion Compare is wake-only (`is: ON`). Compare labels: **turns/crosses/changes state** on the branch **wake** row; **is / is above** on gate rows and inside **`then:`** (no **changes state** in UI there). **G5 ✅:** operator UE/UR **`Cinema rolluik half`** on dashboard (`show_on_dashboard`) — if cinema open % **> 50** → set **50%** open (detail → [`phaseG-integrations.md`](todo/phaseG-integrations.md) § G5).
