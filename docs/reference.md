...
* `app.js`: Master Alpine.js reactive interface store managing SSE channel bindings, connection watchdogs, dynamic client-side uptimes, local UI layout persistence, and JWT role routing. **C27:** `sunriseDisplayText` / `sunsetDisplayText` (local HH:MM + relative) for Admin General Diagnostics and Explorer ℹ popover. **Admin:** `chronologicalTimeline` + `formatPlannedTimelineLabel()` — **`Scene: {name}`** rows (G15 ship). **C37:** Page Lifecycle resume (`visibilitychange` / `resume` / bfcache `pageshow`) force-closes zombie EventSource and reconnects; snapshot fetch abort timeout 20 s; offline overlay must not use `x-cloak`. **B10L:** `offlineStatusLine` milestones under NOT CONNECTED. **C20:** Clear All = `dismissBellAlert` on every visible bell row. **C21:** Explorer `auto_off_countdown` only when device `isOn`.
