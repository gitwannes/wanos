...
* **`GET/POST /api/hue-presets`**, **`PUT/DELETE /api/hue-presets/{key}`** | Admin. **B9A** — CRUD for `hue.presets` in `config_hue_presets.auto.yaml` (text keys; rename = display `name` only; unique display names). Delete **409** when automations still reference `preset: <key>` (usages listed). Create never overwrites an existing key. Hot-reload: **`hue_presets` scope only** — sync `system.hue_presets` + in-memory `config.hue.presets`; **no** full bridge recycle (**B10G Part D** / **G6** row 5).
