...
Stored in `sensor_samples` (`unit` = `C` / `%`) with `climate_hourly` / `climate_daily` rollups. Ingest runs on every successful poll/read; `note_climate_*` applies deadband + max-interval. **`note_climate_reading`** (temp+hum devices): when **either** series is due, **both** are written at the **same timestamp** so dew / AH / Feels-like can pair on `ts`. When T/RH is unchanged, SHT11 and OWM still call history on each poll so **max-interval heartbeats** continue (stable rooms stay on the day chart; state/automation events remain change-only). Sauna probes `20001`/`20002` also refresh `sauna.last_heartbeat_unix` on those stable polls so the HealthMonitor 90s link watchdog does not false-trip. **Z-Wave 76xxx** air-temp probes (often report on-change only; many are temp-only): history on `TEMP_UPDATED` when no RH, plus a bridge heartbeat every **60 s** (10 s while sauna active) using last known RAM value while the node is not `DEAD` (accepts dict or display string `"21.4 °C"` after dual-dispatch). Virtual **`20101`** is also heartbeated from the SHT11 stable-read path so the composite stays continuous when probes are flat.
